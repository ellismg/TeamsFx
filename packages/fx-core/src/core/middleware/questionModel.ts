...
export const QuestionModelMW: Middleware = async (ctx: CoreHookContext, next: NextFunction) => {
  ...
  if (method === "createProject") {
    ...
    const res = await core.tools.ui.showMessage(
      "warn",
      "There will be some configuration change in your project to make it compatible with the latest Teams Toolkit. We recommend to use git for better tracking file changes before migration. Your original project files are archived to the .archive folder. You can refer to .archive.log which provides detailed information about the archive process.",
      true,
      "OK"
    );
