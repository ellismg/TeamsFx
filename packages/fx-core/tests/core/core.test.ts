...
describe("Core basic APIs", () => {
    ...
    it("migrateV1", async () => {
      ...
      migrateV1Params.forEach((testParam) => {
        it(`happy path: migrate v1 project ${testParam.description}`, async () => {
          ...
          sandbox.stub<any, any>(ui, "showMessage").callsFake(
            async (): Promise<Result<string, FxError>> => {
              return ok("OK");
            }
          );
  ...
  async function case4() {
    ...
    if (isV2()) {
      sandbox.stub<any, any>(mockSolutionV2, "getQuestions").callsFake(
        async (
          ctx: v2.Context,
          inputs: Inputs,
      ...
      sandbox.stub<any, any>(mockSolutionV2, "getQuestionsForUserTask").callsFake(
        async (
          ctx: v2.Context,
          inputs: Inputs,
          func: Func,
